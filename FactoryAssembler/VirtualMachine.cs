...
public class VirtualMachine
{
    ...
    public void Reset()
    {
        ...
        IP = 0; CompareFlag = 0; IsHalted = false; IsWaiting = false;
    }
    ...
    private void ExecuteOpcode(string opcode, string[] parts)
    {
        switch (opcode)
        {
            ...
            case "DIV": int d = GetValue(parts[2]); if(d==0) throw new Exception("Div0"); Registers[parts[1]] /= d; break;
            case "CMP": int v1=GetValue(parts[1]); int v2=GetValue(parts[2]); CompareFlag=v1.CompareTo(v2); break;
            case "JMP": if(Labels.ContainsKey(parts[1])) IP=Labels[parts[1]]; break;
            case "JE": if(CompareFlag==0 && Labels.ContainsKey(parts[1])) IP=Labels[parts[1]]; else IP++; break;
    ...
    private int GetValue(string param) { if(int.TryParse(param, out int v)) return v; if(Registers.ContainsKey(param)) return Registers[param]; return 0; }
}