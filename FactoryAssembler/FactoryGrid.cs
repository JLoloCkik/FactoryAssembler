...
public class FactoryGrid
{
    ...
    public FactoryGrid(int cols, int rows)
    {
        Columns = cols;
        Rows = rows;
    }
    ...
    // Csak a kártyákat és a rácsot rajzolja
    public void Draw(Card? draggedCard)
    {
        ...
        foreach (var card in PlacedCards)
        {
            if (card != draggedCard)
            {
                card.Draw(CardWidth, CardHeight);
            }
        }
    }
    ...
    {
        int x = (int)Math.Floor(worldX / CardWidth);
        int y = (int)Math.Floor(worldY / CardHeight);
        return (x, y);
    }
}